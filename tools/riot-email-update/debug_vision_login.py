...
from proxyutil import load_proxy_list
...
def main() -> int:
    ...
    log("init", f"browser_engine={engine} human_mouse={os.getenv('HUMAN_MOUSE', '1')}")
    ...
    for attempt in range(max_attempts):
        ...
        log(
            ...
            f"engine={engine} session={attempt_note['session']}",
        )
            ...
            try:
                ...
                if page_has_riot_oops(page):
                    ...
                    if captcha_visible(page) or "authenticate.riotgames.com" in page.url:
                        ...
                        elif not vision_ok and captcha_visible(page):
                    ...
                    if attempt_note.get("status") not in (
                        "oops_vision",
                        "captcha_fail",
                    ):
                        # --- 05 MFA ---
                        ...
                        if page_has_riot_oops(page):
                            ...
                            if page_logged_in(page):
                                ...
                                attempt_note.setdefault("status", "incomplete")
                                try:
                                    html = page.content()
                                    (DEBUG_DIR / f"a{attempt+1}_06_final.html").write_text(
                                        html
                                    )
                                except Exception:
                                    pass
